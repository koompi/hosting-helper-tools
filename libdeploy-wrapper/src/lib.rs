...
fn get_res(cmd_output: Result<std::process::Output, std::io::Error>, prog: &str) -> Result<(), (u16, String)> {
    match cmd_output {
        Ok(output) => match output.status.success() {
            ...
            false => Err((
                500,
                format!("Your {} has problem. Please reinstall or recheck if usable", prog),
            )),
        },
        Err(_) => Err((
            500,
